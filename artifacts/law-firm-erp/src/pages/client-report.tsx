...
import {
  Loader2, Plus, Trash2, GripVertical, Printer, Save, FileText, ShieldAlert,
  ChevronDown, ChevronUp, ExternalLink, ClipboardList, History, ArrowRight, Mail,
} from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { useAuth } from "@/lib/auth";
import { OFFICE_LOGO_DATA_URI } from "@/lib/office-logo";
import {
...
function safeHref(url: string | null | undefined): string {
  ...
  if (/^data:(application\/|image\/)/.test(trimmed)) return trimmed;
  try {
...
const PRINT_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hdr { background:#111;color:#c9a227;padding:16px 40px;border-bottom:4px solid #c9a227;display:flex;justify-content:space-between;align-items:center;gap:16px; }
  .hdr-logo { width:80px;height:80px;object-fit:contain;border-radius:50%;flex-shrink:0; }
  .hdr-center { flex:1;text-align:center; }
  .hdr-title { font-size:20px;font-weight:800; }
  .hdr-sub { font-size:12px;color:#a08030;margin-top:4px; }
  .hdr-meta { text-align:left;font-size:12px;color:#c9a227;line-height:2;min-width:160px; }
...
function writePrintSkeleton(win: Window, logoDataUri: string) {
  win.document.write(
    `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"/>` +
    `<style>${PRINT_CSS}</style></head><body>` +
    `<div class="hdr">` +
    `<img class="hdr-logo" id="hdr-logo" alt="شعار المكتب"/>` +
    `<div class="hdr-center"><div class="hdr-title">مكتب المحامي ماجد بن سلطان السبيعي</div>` +
  ...
  win.document.close();
  // Inject logo via DOM (not in document.write to avoid encoding issues)
  const logoEl = win.document.getElementById("hdr-logo") as HTMLImageElement | null;
  if (logoEl) logoEl.src = logoDataUri;
}
...
async function openPrintWindow(
  ...
  writePrintSkeleton(win, OFFICE_LOGO_DATA_URI);
...
function BlockCard({
  block, index, total, onChange, onDelete, onMoveUp, onMoveDown, isDragging, dragHandleProps,
}: {
  block: ReportBlock;
  index: number;
  total: number;
  ...
  return (
    <div className={`border rounded-lg bg-card transition-shadow ${isDragging ? "opacity-50 shadow-2xl" : "shadow-sm"}`}>
      <div className="flex items-center gap-2 px-3 py-2 border-b bg-muted/30 rounded-t-lg">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={onDelete}>
          ...
          <div className="space-y-1.5">
            {(block.items ?? []).map((item, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                ...
                {item.extra && <span className="text-xs text-muted-foreground shrink-0">{item.extra}</span>}
              </div>
            ))}
...
export default function ClientReportPage() {
  ...
  const caseNumber = caseData?.caseNumber ?? null;
  const clientName = (caseData as { clientName?: string } | undefined)?.clientName ?? null;
  const caseSubject = caseData?.subject ?? null;
  ...
  const moveUp = (i: number) => {
    if (i === 0) return;
    setBlocks((prev) => { const n = [...prev]; [n[i - 1], n[i]] = [n[i], n[i - 1]]; return n; });
  };
  ...
  const addCustomBlock = () =>
    setBlocks((prev) => [...prev, { id: genId(), type: "custom", title: "قسم مخصص", content: "" }]);
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      toast({ title: `✅ تم إرسال تقرير PDF إلى ${result.sentTo}` });
      loadSavedReports();
    } catch (err: unknown) {
      toast({
        variant: "destructive",
  ...
  const confirmReportDeleteAction = async () => {
    ...
    try {
      await apiFetch(`/api/cases/${caseId}/reports/${reportToDelete.id}`, { method: "DELETE" });
      setSavedReports((current) => current.filter((report) => report.id !== reportToDelete.id));
      setActiveReportId((current) => current === reportToDelete.id ? null : current);
      ...
      toast({
        ...
        description: err instanceof Error ? err.message : "تعذر حذف التقرير. يرجى المحاولة لاحقاً.",
      });
    } finally {
  ...
  return (
    <AppLayout>
        ...
        {view === "builder" && (
          <>
            ...
            <div className="flex gap-2 justify-end pt-2 border-t">
              ...
              <Button
                variant="outline"
                className="gap-2 text-blue-700 border-blue-200 hover:bg-blue-50 hover:border-blue-400"
              ...
              <div className="space-y-3">
                {savedReports.map((r) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm truncate" data-testid={`text-saved-report-title-${r.id}`}>{r.title}</p>
                    ...
                    <div className="flex gap-2 shrink-0">
                      <Button size="sm" variant="outline" className="h-8 text-xs gap-1" onClick={() => loadSaved(r)}>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setPrinting(true);
                          openPrintWindow(r.reportData, r.title, caseNumber, clientName)
                            .finally(() => setPrinting(false));
                        }}
                      >
                      ...
                      <Button
                        ...
                        aria-label={`حذف التقرير ${r.title}`}
                        title="حذف التقرير"
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          if (!open && deletingReportId === null) setConfirmDeleteReport(null);
        }}
      >
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-right leading-relaxed">
              ...
              <span className="font-semibold text-foreground" data-testid="text-confirm-delete-report-title">
                "{confirmDeleteReport?.title}"
              </span>؟
              <br />
          ...
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel
              ...
              data-testid="button-cancel-delete-saved-report"
            >
              إلغاء
            </AlertDialogCancel>
            <AlertDialogAction
              ...
              disabled={deletingReportId !== null}
              data-testid="button-confirm-delete-saved-report"
              onClick={(event) => {
